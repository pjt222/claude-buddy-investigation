...
| **AD–AH — Probe harness + v158–v205 findings & defenses** | Runtime-probe harness, plugin-sync (#140), Cowork remote bridge, credential-helper re-enable (#151), read-guard skip (#152), and the v205 auto-mode exfil-command awareness enrichment (a net-defensive risk hint) | on-demand + per-turn |
...
The three side-systems clip onto the same spine (auth → flags → core runtime) but do not compose. They share OAuth + firstParty + org-scoping; otherwise they are wired independently, built by different teams, at different versions. **The harness grows by accretion, not composition.** Every new subsystem mapped across the v2.1.119–v2.1.206 window (background daemon, MCP channels, the server-controlled config-push primitives, the runtime-probe harness, the v179 system-prompt-string injection #154, the v187 startup-banner styled-text array #155, the v198–v200 background-agent / observer-agent surfaces, the v205 auto-mode exfil-command awareness enrichment, and the v206 staged-call kill-switch + end-conversation agent-lifecycle tool) confirms the pattern at scale: each clips onto the flag spine, and most are gated by a single server-flippable flag.

---

## Finding Status (current — 2026-07-10, session 78)
...
| #154 — a server-pushed system-prompt string flag wired verbatim into the dynamic-section builder, reaching `/v1/messages` system field (role:system) | Critical | wire-confirmed (interactive TUI, v179): injected marker reached the system field verbatim; byte-stable v179→v206 |
...
**The runtime-probe queue (sessions 58–59) is empty;** the v161-177 retro-audit (session 67) re-opened it with two static findings (#151/#152), and the v178–v191 audit (session 69) added #154 (wire-confirmed CRITICAL on an interactive TUI) and #155 (HIGH). The v193–v196 audits (sessions 71–72) produced no new findings and one hardening default-flip; the v197–v200 audits (sessions 73–74) again produced no new findings, no remediations, and no regressions; and the v201–v206 audits (sessions 77–78) once more produced no new findings, no remediations, and no regressions. The only open items are functional WATCHes rather than findings: a default-off, official-marketplaces-only plugin binary-asset provisioning path; the background auto-push / draft-PR whose never-push-to-main / no-force / no-merge boundary is prompt-level, not code-enforced (W-BGPUSH); the observer-agents default-true gate that is inert behind an operator experimental environment variable, to be re-checked only if a future release drops that env gate (W-OBSERVER); the new v203 daemon-side downgrade-refusal guard, which is server-disableable but net-positive (disabling only reverts to the prior behaviour; #113-adjacent); and the v205 auto-mode exfil-command awareness enrichment, which is net-defensive (a risk hint that flags exfil-capable git/gh commands and adds repo public/private visibility plus git-status paths, default-off, first-party-only, and never flips a permission on its own).
...
## Cross-Version Persistence (v2.1.126 → v2.1.206)
...
Extending the window to v2.1.206: the first two remediations of tracked findings landed — #115 at v156 (flag + mechanism removed) and #108 at v179 (the fail-open inversion removed from the binary). Two new findings were added in the v178–v191 window (#154 Critical, #155 High); all other priority-finding anchors remain byte-stable through v206, and v193–v206 added zero findings (v196 carried one hardening default-flip plus a benign, default-off plugin binary-asset provisioning path; v197–v206 added none). Two standing anchors drifted by ±1 at v205 and both decode benign — the #110 field-egress anchor fell by one (a semantics-preserving telemetry-emit hoist; egress unchanged, not remediated) and the #31 AC3 attribution anchor rose by one (a new legitimate producer; the exploited consumer byte-identical). DEFAULT-TRUE moved 32→33 (v202) → 34 (v203) → 35 (v206), 33 boolean + 2 typed, every addition benign and none a safety-gate inversion. The bundled runtime has been stable across the window, but the application build id changes every release — v181→v206 are **genuine per-release builds, not rebuilds of one source tag** (the earlier "unchanged since v181 / identical source tag" framing tracked a runtime-embedded constant that only rotates on a runtime bump, not the app build id); v203 uniquely carried a ~5 MiB code SHRINK (a preview/render engine retired after the diagram path landed) rather than growth, and a shrink can neither add nor mask a finding.
...
1. **The harness grows by accretion, not composition.** The side-systems share OAuth + firstParty + org-scoping and nothing else. Every v119–v200 subsystem clips onto the flag spine independently. The three-direction figure (inside / outside / around) is the predictive shape.
...
3. **Server-flippable single-flag gates are the dominant new-subsystem shape.** Most subsystems mapped across v119–v200 are each gated by one server-controlled flag — activatable per-cohort without a binary release or a release-audit.
