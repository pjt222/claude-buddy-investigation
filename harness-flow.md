...
The three side-systems clip onto the same spine (auth → flags → core runtime) but do not compose. They share OAuth + firstParty + org-scoping; otherwise they are wired independently, built by different teams, at different versions. **The harness grows by accretion, not composition.** Every new subsystem mapped across the v2.1.119–v2.1.217 window (background daemon, MCP channels, the server-controlled config-push primitives, the runtime-probe harness, the v179 system-prompt-string injection #154, the v187 startup-banner styled-text array #155, the v198–v200 background-agent / observer-agent surfaces, the v205 auto-mode exfil-command awareness enrichment, the v206 staged-call kill-switch + end-conversation agent-lifecycle tool, the v208 plugin-instruction server-push override #165, the v215 OR-gated server-selectable prompt-text variants carried as WATCH #166, the v216 question-tool instruction override #168, and the v217 server-pushed subagent recursion-depth ceiling #169) confirms the pattern at scale: each clips onto the flag spine, and most are gated by a single server-flippable flag. The v216–v217 pair sharpens the point in both directions — a **fourth** distinct sink for the same server-pushed-string primitive appeared (#168), while a value that had been a compiled-in constant migrated onto the same channel (#169), so the flag spine keeps absorbing surfaces that used to be fixed at build time.

---

## Finding Status (current — 2026-07-22, session 81)
...
## Cross-Version Persistence (v2.1.126 → v2.1.217)
...
1. **The harness grows by accretion, not composition.** The side-systems share OAuth + firstParty + org-scoping and nothing else. Every v119–v217 subsystem clips onto the flag spine independently. The three-direction figure (inside / outside / around) is the predictive shape.
...
3. **Server-flippable single-flag gates are the dominant new-subsystem shape.** Most subsystems mapped across v119–v217 are each gated by one server-controlled flag — activatable per-cohort without a binary release or a release-audit.
