...
>
> **v2.1.140** (2026-05-13, chain v138→v139→v140; v139 released 2026-05-11): two reader rotations across the chain (a boolean-reader case-flip v139→v140; a typed-reader full identifier rotation each release). Flag delta v138→v140 = **+13 / −4** standalone. **Defense surface EXPANDED — Task()/subagent-launch agentType validation**: three NEW additions on the spawn-time agentType resolution — a `_miss` telemetry event (literal count 0→3 across v138→v140), a `_normalized` telemetry event (0→2), and an `_ambiguous` throw context (0→2). Six prior throw contexts on the same code path stay byte-stable (`_not_found`, `_denied`, `_teammate_background_denied`, `_teams_unavailable`, `_nested_teammate`, `_recursive_fork`). v140 EXPANDS the existing surface with telemetry + a fuzzy normalizer; it does NOT introduce the defense class itself (that has been in the binary since at least v138). Empirically verified live in v140: the not-found error string matches the binary verbatim (pre-existing throw, re-confirmed under v140), and a dash-stripped fuzzy variant ("seniorresearcher") normalizes-and-launches successfully via the new normalizer that collapses dash/case boundaries. The remaining `_ambiguous` and `_denied` paths require synthetic agent-registry collisions or installed deny-rules to trigger — deferred. **SCOPE VERDICT — the new telemetry/normalize layer does NOT cover the SendMessage attribution-forge variant** of the prior ghost-inbox class: the InboxPoller `plan_approval_response` handler that gates session-permission-mode escalation on a sender-string equality check is **byte-stable v138→v140** (canonical-form comparison after identifier normalization across a 600-byte anchored window — identical, including the non-matching-branch log line that was already present in v138). Persistence check across 9 prior findings (raw `_PROTO_` destructure pattern, brief-mode stop-hook reader, third-party-logging gate, harbor PR-status flag, `hookSpecificOutput.updatedInput` nesting, the `--teleport` CLI flag, the forced-downgrade canary primitive, the `sourceToolAssistantUUID` transcript-replay literal, and `parent_tool_use_id`) — all **byte-stable v138→v140 via bounded-context grep**. **No new disclosures filed for v140**; the new layer is a positive instrumentation delta (Task-spawn surface) without remediating the prior SendMessage attribution variant.
>
> **v2.1.140 post-cycle update (2026-05-13)**: The TUI-startup-notice injection finding (filed earlier in the v140 cycle as High) was **promoted to Critical** via a PTY-mounted MITM probe that wire-confirmed raw ANSI escape sequences pass through unsanitized into the rendered notification body. A follow-up demonstration showed the same channel renders OSC 8 terminal hyperlinks as mouse-clickable phishing labels with attacker-chosen URL targets (Ink-side auto-id enhancement amplifies the primitive). Length-cap is ~75 visible characters but ANSI escapes are zero-width and survive the cap. Markdown is rendered as literal text (defensive). A second probe in the same session pushed an invalid (URL-shaped) value through the forced-downgrade primitive's negative branch and wire-confirmed the SemVer-strict parser fires invalid-config telemetry — closing one of that finding's original promotion-gates with positive wire-evidence rather than static decode alone. A third probe documented that the mid-conversation system mechanism's substring-trigger does not fire on non-interactive `--print` mode (predicate gated beyond static substring match — requires conversation-shape preconditions for true-positive triggering).
>
> **v2.1.141 / v2.1.142 / v2.1.143** (2026-05-13 / 2026-05-14 / 2026-05-15): three consecutive stability releases consolidated into a single round-1. Binary sizes 232.5 → 232.6 → 233.0 MB. Reader identifiers rotated: the boolean-reader rotated v140→v141 and again v141→v142, then **held v142→v143** (first hold of the boolean reader since v126); the typed reader rotated every release. Flag delta v140→v143 cumulative **+37 / −10** standalone (per-step: v140→v141 +17/−5, v141→v142 +13/−6, v142→v143 +8/−0 — v143 is a pure-additive release with zero flag removals). DEFAULT-TRUE 15 → 17 → 18 → 18 (+3 net additions across the chain). **Persistence verdict — all 14 priority findings byte-stable v140→v143** via bounded-context grep on semantic literals (feature-flag names, telemetry event names, beta-header strings, SDK Zod schema fields, CLI flags, error-message prose): the TUI startup-notice injection (the Critical from v140), the raw `_PROTO_` destructure egress, the third-party logging gate, the forced-downgrade primitive, the mid-conversation system mechanism, the SendMessage attribution-forge transcript field, the hook-output nesting pattern, the `--teleport` CLI flag, the harbor PR-status flag, the brief-mode stop-hook reader, and the phantom-parent field — all unchanged. Two surface-drift entries decoded as benign code-path consolidation in the remote-control bridge (no semantic remediation). **3 defensive primitives added v141-v142** (orthogonal/adjacent to filed issues, not remediations of any): v141 adds a remote-control bridge event-attestation enforcement layer (enforce / accept-statuses / drop-unverified modes — ORTHOGONAL to the SendMessage attribution-forge path which is local-FleetView, not bridge); v142 adds a settings-hierarchy defense that prevents repo-controllable project-local settings from granting an auto-mode permission default (real hardening of the prompt-via-settings family); v142 also adds telemetry-only model-response keyword detection (NOT a security primitive). **1 NEW CAPABILITY decoded as DEFENSE-WORKING** (not filed as a finding): v143 introduces a multi-store team-memory mirror gated by a JSON env var. Static decode confirms triple-gate eligibility (trust-dialog + a default-FALSE GrowthBook flag + OAuth presence) + a path-validator that requires server-relative paths and rejects external-origin pivots + server-side ACL enforcement + per-store mode controlling push direction + a path-confinement guard on disk writes; attacker-controlled env var only chooses WHICH server-authorized store to mirror, cannot grant authorization victim doesn't already have. Documented as defense-in-depth working as designed. **Zero new disclosures filed v141-v143. Zero remediations observed.**
>
> **Live security tally (2026-05-18 GH-label re-derivation):** 12 critical / 30 high-priority / 46 medium-priority / 8 low-priority = **96 severity-labeled across 127 repo issues** (counts stable since v140 cycle close 2026-05-13 — no new issues filed in the v141-v143 cycle); audit-baseline-tally lineage 29 (14 audit + 15 post-audit through #127). Post-audit additions reside in the private repo's issue tracker.
