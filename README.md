...
> **Status (2026-05-27, session 61 docker close):** The companion UI was **removed from the binary in v2.1.97** (built April 8), but the **`buddy_react` API is still alive** server-side. The **advisor system** (`advisor_20260301`) is code-complete since v2.1.96, dark-launched behind a server-side feature flag (see `advisor-architecture.md`). Since v2.1.118 the investigation has broadened into a rolling per-version security audit of the wider Claude Code harness. **Current binary: v2.1.152** (npm `latest`, BUILD 2026-05-26). All 20 testable priority-finding literals are **byte-stable v145→v147→v148→v152 with no remediation observed**. Session-61 docker session promoted two findings to CRITICAL with end-to-end wire-confirmation: **#113** (forced-downgrade — the on-disk binary is actually swapped to an attacker-chosen older version via the auto-updater + npm install) and **#136** (a server-pushed plugin-allowlist — the production server pushes a non-empty allowlist by default that propagates the OAuth bearer into the env of any on-list plugin's hook subprocess). Live security tally (`docs/counts.js`, derived 2026-05-27): **14 critical / 29 high / 46 medium / 8 low** across 136 repo issues.
>
...
>
> **Per-version timeline (v2.1.118 → v2.1.152).** Each line points to the `results/` file with the full analysis.
>
...
- `SECURITY-AUDIT.md` — original 14-finding security audit across CLI, MCP server, capture system, and docs site. The live tally has since grown well beyond this with post-audit harness-level findings: `docs/counts.js` tracks an audit-baseline lineage and a live-GH re-derivation (2026-05-27) of **14 critical / 29 high / 46 medium / 8 low = 97 severity-labeled across 136 repo issues**. `docs/counts.js` is the single source of truth — see its two-axis tally.
...
The `buddy_react` API and the dark-launched advisor have remained stable through to the current binary (**v2.1.152**); the companion UI has not returned. The harness audit since v2.1.118 has rolled across every published release except a handful of skips (v2.1.120/122/124/125/127/130/134/135/136/137/139/146/149/150/151).
...
- Rolling per-version harness security audit (97 severity-labeled findings as of v2.1.152; original tooling audit was 14)
...
Since v2.1.118 the rolling harness audit has filed many more — the live tally (`docs/counts.js`, derived 2026-05-27) is **14 critical / 29 high / 46 medium / 8 low across 136 repo issues**. The current critical set centers on server-controlled config-push channels: **#106** (model-context injection via a Stop-hook reminder override), **#108** (a permission-classifier safety inversion), **#110** (raw plugin/skill/marketplace identifier field-level egress), **#127** (unsanitized terminal-notification injection — wire-confirmed session-59), plus **#31 AC3** (subagent ghost-inbox / attribution forgery — still undefended in v2.1.152). **#113** (a forced-downgrade primitive — wire-confirmed session-59) and **#136** (server-pushed plugin-allowlist OAuth-bearer egress into plugin hook subprocess env — filed session-60) are the headline HIGHs.
