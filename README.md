...
>
> **Per-version timeline (v2.1.118 → v2.1.177).** Each line points to the `results/` file with the full analysis.
>
...
- `SECURITY-AUDIT.md` — original 14-finding security audit across CLI, MCP server, capture system, and docs site. The live tally has since grown well beyond this with post-audit harness-level findings: `docs/counts.js` tracks an audit-baseline lineage and a live-GH re-derivation (2026-06-15) of **14 critical / 35 high / 53 medium / 11 low = 113 severity-labeled across 152 repo issues**. `docs/counts.js` is the single source of truth — see its two-axis tally.
...
The `buddy_react` API and the dark-launched advisor have remained stable through to the current binary (**v2.1.177**); the companion UI has not returned. The harness audit since v2.1.118 has rolled across every published release except a handful of skips (v2.1.120/122/124/125/127/130/134/135/136/137/139/146/149/150/151/154/155/157) and the two unpublished versions in the v161-177 window (never released to npm).
...
- Rolling per-version harness security audit (113 severity-labeled findings as of v2.1.177; original tooling audit was 14)
