...
> - **v2.1.132** (2026-05-07): stability release. **Reader rotation** — both default-true and 3-arg eval readers from v131 unified into a single eval reader. Per the cross-version methodology rule (semantic literals over minified-identifier patterns), this is cosmetic noise. Flag delta **+23/-5** (net +18); DEFAULT-TRUE 15→14 (one UI keybinding flag removed). **Forward-compat REPL-input slot** byte-IDENTICAL destructure-strip context (advisor catch on count vs. proof — bounded-context grep confirms same allowlist as v131; only function-name minification rotated). **Second-Critical trigger on the v129 destructure-rename leak NOT fired**. **16 priority literals byte-stable** v131→v132 across 9 existing findings (hook system, telemetry classifier, share-onboarding component, datadog gate flag, classifier-fail-open prose, brief-stop-hook reader, team-onboarding reader, teleport telemetry/CLI, REPL-input slot). **3 new topic-relevant flags decoded statically — all telemetry-only, not new-disclosure-class**: an auto-mode classifier `fallback_to_ask` event (4 fallback conditions), a team-onboarding `share_deleted` event (empty payload), a goal-lifecycle `stop_hook_removed` event (NOT brief-stop-hook removal — different code path). One escape-telemetry event removed (ambiguous tightening vs. loosening). **Zero remediations across 5-version persistence** (v126/v128/v129/v131/v132). Net direction: surface ADDED > surface REMOVED.
>   - **Wire-confirmation triple-probe (2026-05-07)**: single MITM run captured all three wire channels. **Datadog third-party finding** (v131 baseline) re-confirmed byte-stable on v132: 50 telemetry events per ~30s session, 100% PII envelope (session/subscription_type/device-hash), 44/50 events leak per-feature subsystem-fingerprint name covering plugin/MCP/LSP/skill/hook/memory/git/output-style subsystems; gate ruleId byte-stable v126→v132 (production rule, not churn). **Telemetry destructure-rename finding** wire-IDENTICAL across 4 versions (v129/v131/v132): same 356 raw skill names, 9 plugin names (2 unique), 9 marketplace names (2 unique) on bootstrap event_logging batch — zero redaction, zero remediation. **GrowthBook server-side state**: 224 resolved features byte-stable count, minor source-class drift (force 48→47, default 162→161, experiment 14→16). All 3 new v132 topic-relevant flags govern by GrowthBook default (server-flip not yet active for this account). The brief-mode stop-hook GrowthBook string-flag (v126 finding) governs by empty-default for this account — v126 injection vector remains structurally live (one server-flip away from triggering for this account population). One server-config flag force-flipped pushes a 30-plugin list to the client but is byte-absent from the binary — config-staging artifact, not consumed.
