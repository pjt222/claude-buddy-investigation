...
> **Status (2026-05-29, session 62 flag-diff):** The companion UI was **removed from the binary in v2.1.97** (built April 8), but the **`buddy_react` API is still alive** server-side. The **advisor system** (`advisor_20260301`) is code-complete since v2.1.96, dark-launched behind a server-side feature flag (see `advisor-architecture.md`). Since v2.1.118 the investigation has broadened into a rolling per-version security audit of the wider Claude Code harness. **Current binary: v2.1.156** (npm `latest`, BUILD 2026-05-28). 15 of 16 priority-finding literals are **byte-stable v152→v153→v156**; **#113** forced-downgrade telemetry expanded to a 2nd call site (same primitive, still CRITICAL) and **#115** (a mid-conversation-system substring-trigger mechanism) was **REMEDIATED + CLOSED** — the flag and its mechanism were removed from the binary in v156. v156 also ships **Opus 4.8** (`claude-opus-4-8`), an **"ultracode"** max-effort mode, and a **workflows keyword opt-in**. Two findings remain wire-confirmed CRITICAL from session-61: **#113** (forced-downgrade — the on-disk binary is actually swapped to an attacker-chosen older version via the auto-updater + npm install) and **#136** (a server-pushed plugin-allowlist — the production server pushes a non-empty allowlist by default that propagates the OAuth bearer into the env of any on-list plugin's hook subprocess). Live security tally (`docs/counts.js`, re-derived 2026-05-29): **14 critical / 29 high / 46 medium / 8 low** across 136 repo issues — unchanged.
>
...
The `buddy_react` API and the dark-launched advisor have remained stable through to the current binary (**v2.1.156**); the companion UI has not returned. The harness audit since v2.1.118 has rolled across every published release except a handful of skips (v2.1.120/122/124/125/127/130/134/135/136/137/139/146/149/150/151/154/155).
...
- Rolling per-version harness security audit (97 severity-labeled findings as of v2.1.156; original tooling audit was 14)
