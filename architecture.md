...
**Date**: 2026-04-02 (updated 2026-09-02)
**Version**: 1.7
...
>
> **v2.1.242 → v2.1.258 (current, sessions 88–90):** Audited coverage now runs **v2.1.89 → v2.1.258** — the installed binary (npm `latest` = `next` = 258) — and the *zero unaudited gaps* property is no longer a sentence anyone advances by hand: a **generated** per-version coverage table, built from each write-up's own front matter rather than from filenames or prose, emits a row for every released version and a **failing check** when a released version has no document, so the property is now *computed*. Six release slots in this range (v2.1.244, v2.1.249, and v2.1.253–v2.1.256) were never published for this platform, so anything introduced and reverted inside them is invisible and always will be. The companion and harness *architecture* documented in this file is unchanged across the window. **Census and tally:** DEFAULT-TRUE gates now stand at **109 (107 boolean + 2 typed)**, up from **76 (74 boolean + 2 typed)** at v2.1.241 — and the boolean figure was **corrected mid-window**, because a *third* reader shape (a hoisted flag name carrying an inline default) proved invisible to two independent instruments that had each been trusted as a cross-check (**#200**); the live severity tally is **15 Critical / 51 High / 77 Medium / 19 Low across 205 issues**. Per-version detail continues in `digest.md`, `README.md`, `docs/counts.js`, and the `results/` files.
>
> **v2.1.242 → v2.1.246 (sessions 88–89)** add **two High findings on one new surface plus a method finding**, and are otherwise a zero-finding range whose real content is a pair of counting traps. **v2.1.242** is the release the first pass had skipped — **+34.9 MB** and a bundle re-split from 11 modules to roughly 1,385 — and it carries **#195 (High)**: a new plugin **hook-module runtime** lets a registered handler **substitute** the tool description sent to the model, and prompt-section text, rather than *append* to it — no delimiter, no attribution, and validation amounting to a type check plus a 32,000-character cap. It sits behind a **default-off internal gate that is absent from the served configuration cache**, so the server can arm it. It was filed High with the promotion gate being **provenance** — whether a server-influenced plugin can carry such a module — not shape; session 89 **answered that gate without clearing it**: registration is *not* restricted to locally-installed plugins, but no path lets the server supply the module's **content**, so the server arms and the plugin distribution channel supplies. **#201 (High)** records that **v2.1.251 widened the same surface**: one substitution kind rewrites the **trailer block that lands in every commit message and pull-request body** — the first reach in this family that **leaves the machine** — and another replaces whole skill bodies. **#193 (High, method)** is the reusable part: the reason #195 was missed is that every agent chose its targets from the **standing anchor table**, which is by construction a list of the *previous* window's literals, so the release's largest new subsystem was invisible to the entire method; there is still **no census of the model-context surface as a class**. The rest of the range is **zero findings and two counting traps, both of which had been silently corrupting earlier censuses.** First, the extraction step read **7-bit single-byte literals only**, so every census this project had ever run was blind to the binary's UTF-16 text — and the bias runs toward **false removals**, which read as remediation: a cached diff asserting 68 removed API endpoints was wrong on **65** of them. The extractor now emits both encodings and the whole cache was re-acquired. Second, a raw occurrence count is *source copies* **plus one bytecode-constant-pool copy per referencing code block**, and the pool term belongs to the **build**, not to the code — it moves whenever the bundler re-chunks. All **eight** anchor "drops" at v2.1.246 are packaging: none was a remediation, none was even a refactor, and the bundled-runtime revision is **not** the predictor. A dedicated tool now makes that split *before* any claim is drawn.
>
> **v2.1.246 → v2.1.258 (sessions 89–90)** cover twelve release slots, seven of them published, and close with **one escalation of a standing finding, zero new vectors, and an unusually security-positive posture.** **#182 escalated from server-flippable to shipped default.** The gate whose premise was "the server could flip this on" **flipped its compiled default at v2.1.248**, so that premise is now what ships. The mechanism is unchanged — the cross-session inbound gate still trusts a permission-mode field authored by the **sender**, which the binary's own schema concedes is only "as declared by" the sending host — but the reach changed, and **the channel inverts**: the server would now have to push the flag **off** to restore the human-approval hold for a bypass-class receiver. **#206 (High)** — of the messages in the cross-session approval family, the response that **sets the receiver's permission mode** is the only one carrying **no sender binding at all**, while every sibling pins the sender identity to the envelope; this **overturned a "verified non-finding"** recorded earlier in the same window, and the evidence that refuted it was already in hand when the wrong conclusion was written. **#203 (High)** — the system prompt is **recorded once and reused verbatim** on every later request and on resume, with a corrected relaunch ignored until compaction; that **multiplies the lifetime of every injection path** tracked in this project rather than adding a new one. **#196, #197, #198, #199 (High)** and **#200, #204 (Medium)** add four further server-pushed-string paths into model context plus two method and scope findings; **#199** is the sharpest shape, because serving its flag **TRUE turns an auto-mode consent rule OFF** by excising a named rule from the safety classifier's own instruction text — so a census that reads compiled defaults scores a **consent-removing** control as benign. **#154 was upgraded on evidence rather than on severity:** server-authored prompt text for that finding was found sitting in the **local cache of served configuration values on disk**, verbatim in a live session's system prompt, while the same text is **absent from the current binary** — so the binary cannot be its source; that delivery arm is in routine production use and is **cohort-targeted**. **#202 (High)** was filed on v2.1.251's tracing fix, re-scoped out of High by adversarial review, then **restored in session 90** once the deciding read was actually run: the project-scope environment filter consults **its own blocklist**, which omits the content-logging telemetry family, while the collection that *does* carry those names guards administrator settings tiers on a different path entirely — scoped as a **stock-machine** finding, since on a managed deployment a higher-trust telemetry claim reclaims the destination setting from lower-trust scopes and on an unmanaged one it does not. **Security-positive movement is worth stating because it is rare:** v2.1.251 ships **five** upstream fixes, all verified still present at v2.1.258, and **v2.1.257 deletes a bundled path-walking dependency outright** — the resulting drop in path-resolution call sites *reads* like a regression and is the opposite, because the replacement primitive is a **file-descriptor and handle check** rather than path re-resolution, and every defence axis measured **grew**. **Five method rules were earned in this range**, all of which change how future counts must be read: a flat occurrence count never proves a **default** is unchanged; a default is **not a state**, so every census must be diffed against the served configuration cache before severity is assigned (§14.5); a literal count of **zero** is evidence of absence from the string pool and never proof of absence from the code; a property of the **innermost stage** of a composed filter is not a property of the filter; and in minified output a **negated zero is TRUE**, so it marks dead-code-elimination residue rather than an unreachable branch.
...
## §14 Server-Flippable Control Plane (v2.1.118 → v2.1.258)
...
- **#182 (High, new in the v2.1.221–v2.1.241 range)** — a boolean flag makes a new cross-session message gate trust a field the **sender supplies about itself**, converting a hold-for-human-review into an automatic accept for a receiver running with permission prompts bypassed. This contradicts the subsystem's own in-source contract, which states in as many words that the sender-supplied origin field is forgeable by any process running as the same user and must never be used to key identity. **Escalated at v2.1.248:** the flag's *compiled default* flipped on, so the sender-trusting acceptance is now what ships rather than what the server could push — and the channel inverts, since restoring the human-approval hold for a bypass-class receiver would now require the server to push the flag **off** (see §14.7).
...
- **#195, #196, #197, #198, #199, #201 (High, new in the v2.1.242–v2.1.258 range)** — six further server-armed paths that put externally-authored text into model context, including the first that **substitutes** model-facing text rather than appending to it and the first whose output **leaves the machine**; **#199** inverts the usual polarity, since serving its flag *true* **removes** a named consent rule from the auto-mode safety classifier's own instruction text. Summarised in §14.7.
- **#200, #204 (Medium, new in the v2.1.242–v2.1.258 range)** — a census-instrument gap (a third flag-reader code shape invisible to two independent instruments) and a scope finding; see §14.7.
...
### 14.7 Sessions 88–90 (v2.1.242 → v2.1.258)

Three things happened in this range that change how §14 should be read.

**A new model-context substitution surface, and the anchor-table blindness that hid it (#195, #201, #193).** The plugin **hook-module runtime** introduced at v2.1.242 is the first mechanism in this codebase that lets a registered handler **replace** model-facing text — a tool description, a prompt section, and (from v2.1.251) whole skill bodies and the trailer block appended to commit messages and pull-request bodies — rather than append to it. Validation is a type check plus a 32,000-character cap; there is no delimiter and no attribution, so the substituted text is indistinguishable from first-party text once it reaches the model. It is gated by a **default-off internal gate that does not appear in the served configuration cache**, which means the server can *arm* the surface while the plugin distribution channel supplies the content — a two-party shape the rest of §14 does not have. The trailer-block variant is the first reach in **this** family whose output **leaves the machine** — it lands in a durable third-party artifact (git history, a pull-request body), not merely in a request to the model. The method lesson (**#193**) generalizes past this window: every audit unit chose its targets from the standing anchor table, which is by construction a list of the *previous* window's literals, so the largest new subsystem in the release was invisible to the method — and the repository still has no census of the model-context surface **as a class**.

**Escalation without a code change (#182), and a sender-binding gap beside it (#206).** The cross-session inbound gate whose finding text read "the server could flip this on" had its **compiled default flipped on at v2.1.248**. Nothing about the mechanism moved — the gate still trusts a permission-mode field authored by the **sender**, which the binary's own schema concedes is only "as declared by" the sending host — but the premise is now the shipped state, and **the channel inverts**: restoring the human-approval hold for a bypass-class receiver would now require the server to push the flag *off*. In the same family, **#206** found that the one message that **sets the receiver's permission mode** carries **no sender binding at all**, while every sibling pins the sender identity to the envelope. #206 overturned a *verified non-finding* written earlier in the same window on evidence that was already in hand — the strongest available argument that a confidently-recorded negative deserves the same verification budget as a positive.

**The measurement instruments were wrong, in the direction that reads as good news.** Two defects were found in the census tooling that had been silently corrupting earlier windows. The literal extractor read **7-bit single-byte text only**, so every census this project had ever run was blind to the binary's UTF-16 strings — and that bias produces **false removals**, which look exactly like remediation (one cached diff asserting 68 removed API endpoints was wrong on 65 of them). And a raw occurrence count is source copies **plus one bytecode-constant-pool copy per referencing code block**; the pool term is a property of the **build**, not of the code, and moves whenever the bundler re-chunks — all eight anchor "drops" at v2.1.246 are packaging, none a remediation. Both are now tooled: the extractor emits both encodings (and the cache was fully re-acquired) and a dedicated splitter separates a count delta from a packing delta before any claim is drawn. Alongside them, this range is genuinely **security-positive** — v2.1.251 ships five upstream fixes, all still present at v2.1.258, and v2.1.257 removes a bundled path-walking dependency outright, replacing path re-resolution with a file-descriptor and handle check; the drop in path-resolution call sites reads like a regression and is the opposite.

---

*§13–§14 added 2026-05-20 (Session 59), reflecting the v2.1.118→v2.1.145 runtime-probe era. Function and flag-reader identifiers are minified and rotate per release — cross-version claims anchor on string-pool literals. Sources: `results/runtime-probes-session-58.md`, `results/runtime-probes-session-59.md`, `results/v2.1.144-145-round-1-flag-delta.md`, `results/docs-gap-analysis-2026-05-20.md`. Finding severities mirror `docs/counts.js`. §14.5–§14.6 added 2026-08-25 (session 87), covering v2.1.218→v2.1.241; sources: the session-83/84/87 window write-ups in `results/`. §14.7 added 2026-09-02 (session 90), covering v2.1.242→v2.1.258; sources: the session-88/89/90 window write-ups in `results/`.*
