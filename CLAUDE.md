...
- `SECURITY-AUDIT.md` — original 14-finding tooling audit (1 CRITICAL, 3 HIGH, 5 MEDIUM, 3 LOW, 1 OBSERVATION). The live security tally has since grown well beyond this: `docs/counts.js` tracks an audit-baseline lineage and a live re-derivation of 14 critical / 35 high / 53 medium / 11 low across 152 repo issues (re-derived 2026-06-15). `docs/counts.js` is authoritative — do not contradict it.
...
- **#31 AC3** (subagent ghost-inbox / attribution-forgery class) **remains UNDEFENDED as of v2.1.177.** A v145 guard addresses skill self-recursion only, orthogonal to the inbox path. Anchor literals byte-stable v145→v177. (The v160 sub-agent-output security-warning surface is an output content-check, orthogonal to inbox attribution — watch W5; a v176 forged-turn detector is telemetry-only and adds no new defense to the inbox path.)
