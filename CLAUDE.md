...
- `SECURITY-AUDIT.md` — original 14-finding tooling audit (1 CRITICAL, 3 HIGH, 5 MEDIUM, 3 LOW, 1 OBSERVATION). The live security tally has since grown well beyond this: `docs/counts.js` tracks an audit-baseline lineage and a live re-derivation of 14 critical / 31 high / 47 medium / 9 low across 140 repo issues (re-derived 2026-05-31). `docs/counts.js` is authoritative — do not contradict it.
...
- Binary at `~/.local/share/claude/versions/`. Version chain probed: v2.1.90/v2.1.92 (companion), v2.1.96–v2.1.100 (advisor), v2.1.101/v2.1.104 (loop system), v2.1.105–v2.1.114 (harness/gate-surface), v2.1.116–v2.1.160 rolling per-version flag-diff (skips: v2.1.120/122/124/125/127/130/134/135/136/137/139/146/149/150/151/154/155/157). **Current = v2.1.160** (npm `latest`, BUILD 2026-06-01). v158 adds an opt-in plugin-sync leg (an environment-variable gate, default off) atop the pre-existing skills-sync framework — **#140 HIGH**: it headlessly registers an org's synced plugins' MCP servers (including subprocess-spawning ones) with no consent prompt in the sync caller, asymmetric to the skills-sync sibling which suppresses hook registration. **v159 trivial; v160 = major feature release** (+2.16MB binary, runtime unchanged ⇒ ~1.76MB new code) — a flag-name-only diff (+13/-0) structurally HID the content: 39 new client env-var literals + a **Cowork remote-environment bridge** (runner-side only — activates as a cloud remote environment, not the local CLI; its egress-gateway allow/deny policy is unverified, logged as runtime-probe target W2) + a **two-stage auto-mode safety classifier** (fail-CLOSED = safety strengthening, the opposite of the #108 inversion). **No new finding filed.** Methodology rule reinforced: the non-flag diff (client env vars / server-pushed client-cache keys / API endpoints / runtime version) is now mandatory per release; reconcile binary-size delta vs strings-byte delta. DEFAULT-TRUE 22 (19 boolean + 3 typed), re-derived directly from the v160 binary, unchanged v158→v160.
...
- **#31 AC3** (subagent ghost-inbox / attribution-forgery class) **remains UNDEFENDED as of v2.1.160.** A v145 guard addresses skill self-recursion only, orthogonal to the inbox path. Anchor literals byte-stable v145→v160. (The v160 sub-agent-output security-warning surface is an output content-check, orthogonal to inbox attribution — watch W5.)
