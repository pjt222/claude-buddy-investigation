...
- `SECURITY-AUDIT.md` — original 14-finding tooling audit (1 CRITICAL, 3 HIGH, 5 MEDIUM, 3 LOW, 1 OBSERVATION). The live security tally has since grown well beyond this: `docs/counts.js` tracks an audit-baseline lineage (30 items: 7 critical / 9 high / 10 medium / 3 low / 1 observation) and a live re-derivation of 14 critical / 37 high / 53 medium / 11 low across 155 repo issues (re-derived 2026-06-30, session-72). `docs/counts.js` is authoritative — do not contradict it.
...
- **#31 AC3** (subagent ghost-inbox / attribution-forgery class) **remains UNDEFENDED as of v2.1.196.** A v145 guard addresses skill self-recursion only, orthogonal to the inbox path. Anchor literals byte-stable v145→v196. (The v160 sub-agent-output security-warning surface is an output content-check, orthogonal to inbox attribution — watch W5; a v176 forged-turn detector is telemetry-only and adds no new defense to the inbox path.)
