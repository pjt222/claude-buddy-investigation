...
- Binary at `~/.local/share/claude/versions/`. Version chain probed: v2.1.90/v2.1.92 (companion), v2.1.96–v2.1.100 (advisor), v2.1.101/v2.1.104 (loop system), v2.1.105–v2.1.114 (harness/gate-surface), v2.1.116–v2.1.158 rolling per-version flag-diff (skips: v2.1.120/122/124/125/127/130/134/135/136/137/139/146/149/150/151/154/155/157). **Current = v2.1.158** (npm `latest`, BUILD 2026-05-29). All priority-finding literals byte-stable v156→v158, no remediation. v158 adds an opt-in plugin-sync leg (an environment-variable gate, default off) atop the pre-existing skills-sync framework — **#140 HIGH**: it headlessly registers an org's synced plugins' MCP servers (including subprocess-spawning ones) with no consent prompt in the sync caller, asymmetric to the skills-sync sibling which suppresses hook registration. DEFAULT-TRUE boolean net-flat at 19 (one new UI default added, one flipped to default-false; 22 total with the 3 typed — the counts.js scalar was corrected, stale since v152).
...
- **#31 AC3** (subagent ghost-inbox / attribution-forgery class) **remains UNDEFENDED as of v2.1.158.** A v145 guard addresses skill self-recursion only, orthogonal to the inbox path. Anchor literals byte-stable v145→v158.
