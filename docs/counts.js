...
window.VIZ_COUNTS = Object.freeze({
  ...
  security: {
    ...
    gh_label_counts: {
      critical: 15,   // session-82: +1 = #168 PROMOTED High to Critical on wire confirmation. An injected value on the server-to-client configuration channel arrived in the outbound messages-endpoint request inside the question tool's own description field, spliced mid-instruction between two paragraphs of the genuine text with no delimiter or attribution; the model-eligibility-gated sibling arrived at its default in the same capture, confirming the unconditional read on the wire. Joins the earlier server-pushed-string primitives at the Critical tier.
      high: 52,   // session-90 close-out: +1 -- a defect in the publish gate itself: its primary layer clears a token as documented on a SUBSTRING match, so a bare token that is only a prefix of a published name passes. Found when the two gate layers disagreed during this very sync and the primary one was wrong. PRIOR sessions 88-90: +10, re-derived DIRECTLY from issue labels rather than incremented. #195 (a plugin runtime whose registered handler SUBSTITUTES the tool description sent to the model and prompt-section text, behind a default-off gate the server can arm), #193 (no census of the model-context surface as a class exists at all), #196/#197/#198/#199 (four further server-pushed-string paths into model context; #199 INVERTS POLARITY -- serving it TRUE removes a consent rule from the safety classifier's own instruction text), #201 (the same substitution surface widened to the trailer block that lands in every commit message and pull-request body, the first reach in this family that LEAVES THE MACHINE), #202 (the project-scope environment blocklist omits the content-logging family; filed High, re-scoped out by adversarial review, RESTORED in session-90 once the deciding read was run, and scoped as a stock-machine finding), #203 (the system prompt is recorded once and reused verbatim on every later request and resume), #206 (of the cross-session approval messages, the one that sets the receiver's permission mode is the only one carrying no sender binding). PRIOR session-87: +3 = #176 (runner argument denylist), #181 (destructive external-tool auto-approve widening), #182 (cross-session gate trusts a sender-supplied field; filed High, downgraded by us to Low, then RESTORED on served-configuration evidence). Re-derived directly from issue labels. PRIOR session-82: -1 = #168 promoted to Critical (see above). It was filed High at session-81 only because the reach was call-site-derived: a server-pushed configuration STRING with an empty default interpolated verbatim into the instruction text of the built-in multiple-choice question tool, validated only by a type check and a whitespace trim (no length cap, no schema).
      medium: 77,  // sessions 88-90: +9, re-derived DIRECTLY from issue labels. Includes #200 (a third flag-reader shape is invisible to two independent instruments, so the default-true population was a PRE-EXISTING undercount) and #204 (two new model-switch hooks that can cancel a switch and carry a documented model-context field). PRIOR session-87: +4 = #177 (identity binding removable from a live remote-control channel), #178 (a cross-tool permission-response confusion with a residual still live), #184 (a fast path auto-approving writes outside the declared working set), #185 (the served-configuration method gap). Includes +3 from sessions 84-86 (#172, #173, #174). PRIOR session-81: +1 = a tooling/process issue recording a second redaction-enforcer gap in this project's own publish pipeline, found during this window's mirror sync and fixed the same session: one published data file is machine-redacted by nothing (it is hand-edited forward each window), and minified identifiers are covered only by an enumerated list rather than a structural shape check. NOT a harness finding. PRIOR session-80: +1 = a tooling/process issue recording a redaction-enforcer gap in this project's own publish pipeline (found and fixed the same session), NOT a harness finding.
      low: 19,   // sessions 88-90: +1. PRIOR session-87: +2 = #179 (four runner-scoped low items) and #183 (a new class of local execution: informational plus two low). Includes +1 from session-84 (#171, a dead retry latch on the permission-classifier path; fail-CLOSED, so an availability regression and not an authorisation inversion). PRIOR session-81: +1 = #169, the subagent recursion-depth ceiling became a server-pushed integer with NO upper clamp (operator env, else server value, else local default). NOT a permission or sandbox inversion - resource breadth only - and the shipped posture IMPROVED, since the default fell from a hardcoded 5 to 1. Filed for the unbounded remote knob, not the default. PRIOR session-80: +1 = a new watch-tracker issue for the server-selectable prompt-variant gates (a tracker with a promotion tripwire, NOT a severity finding - two independent passes returned watch-only). Zero new harness findings this window.
      labeled_total: 163,   // 15C + 52H + 77M + 19L (session-90 close-out, re-derived live). PRIOR 162 = 15C + 51H + 77M + 19L (session-90, re-derived live 2026-09-02). PRIOR 142 = 15C + 41H + 68M + 18L (session-87)
      unlabeled_by_severity: 44,  // 207 - 163 (session-90 close-out). PRIOR 43 = 205 - 162 (session-90). PRIOR 42 = 184 - 142 (session-87)
      repo_total: 207,  // session-90 close-out: +2, both about this pipeline rather than the product -- the hand-maintained count files that keep drifting, and the gate defect above. PRIOR sessions 88-90: +21. Severity tally re-derived DIRECTLY from issue labels, not incremented. PRIOR session-87: +15 across sessions 84-87. Severity tally re-derived DIRECTLY from issue labels, not incremented. PRIOR session-81: +3 total = #168 (HIGH) and #169 (LOW), BOTH genuine harness findings - the first non-zero-finding window since v208 - plus a tooling/process issue for the redaction gap found during the mirror sync. PRIOR session-80: +2 total = #166 (prompt-variant watch tracker) and #167 (a tooling/process issue recording a redaction-enforcer gap in this project OWN publish pipeline, found and fixed the same session). Harness findings this window: ZERO.
  ...
  //   4. GrowthBook feature cache (the served-configuration cache in ~/.claude.json)
  ...
  investigation: {
    agents_deployed: "21+",
    waves: 16,
    current_binary: "2.1.258",   // sessions 88-90 (2026-09-02): v241 to v258 audited, and audited coverage now REACHES the installed binary with zero unaudited gaps -- a COMPUTED property backed by a generated per-version coverage table and a failing check, not a claim. Twelve release slots in the last window, seven published for this platform; several releases never shipped, so anything introduced and reverted inside them is invisible and always will be. ONE ESCALATION, no new vector: the cross-session inbound gate's flag flipped its COMPILED DEFAULT at v2.1.248, so #182 no longer depends on a server push -- the gate trusts a sender-authored permission-mode field BY DEFAULT, and the server would now have to push it OFF to restore the human-approval hold. THE WINDOW'S STRONGEST EVIDENCE IS ON DISK, NOT IN THE BINARY: server-authored prompt text for #154 sits in the local config cache, verbatim in a live session's system prompt, while the same text is ABSENT from the current binary -- so the binary cannot be its source and that delivery arm is in routine, cohort-targeted production use. System-prompt content has moved from the binary (auditable, versioned) to the network (neither). Ten issues filed. SECURITY-POSITIVE, which is rare enough to state: v2.1.251 shipped five upstream fixes, all verified present at v2.1.258, and v2.1.257 deletes a bundled path-walking dependency outright -- the resulting drop in path-resolution call sites reads like a regression and is the opposite, since the replacement primitive is a descriptor and handle check rather than path re-resolution. Session-90 then answered the one question the review had left open and restored #202 to High. PRIOR session-87 (2026-08-25): v221 to v241 DECODED in one pass \u2014 20 windows, 21 published versions. TEN issues filed (#176-#185); coverage moves to v2.1.241, the first move since v220. THE HEADLINE IS A METHOD GAP, NOT A FINDING (#185): every flag census this project had run read the DEFAULT out of the binary and assigned severity from it, which is the wrong question for a server-controlled configuration channel \u2014 the client's own local cache of what the server ACTUALLY SERVED had never been diffed against a census. 17 of the 55 newly-added default-off gates are switched ON for this account, four of them paths carrying externally-authored text into model context; the inverse check is clean (of 76 default-on gates only three are served off, none a permission decision). It reversed a severity call mid-session. SCOPING ERROR caught by the pass's own completeness critic: the planned sweep covered only the 35 new default-ON gates, but every wire-confirmed finding in the server-push injection lineage is an empty-or-false-default value the server FILLS IN, so the permissive state is reached by a push and not a withhold. The 55 default-OFF gates were then swept properly (54/54; adversarial verification returned 8 confirmed, 21 confirmed-but-overgraded, 2 refuted). TWO High on the permission/injection path: #176 (a runner applies server-supplied command-line arguments to its spawned child through a DENYLIST rather than an allowlist, leaving reachable one argument that appends server-controlled text to the child's system prompt and one that moves the child out of permission checking; a sibling bridge in the SAME binary uses a strict allowlist that predates it) and #182 (a server-controlled flag makes a new cross-session gate trust a field the SENDER supplies about itself, converting hold-for-review into automatic accept). #181 is a third (a default-off flag converting a mandatory human approval on destructive external-tool calls into a classifier decision \u2014 currently served OFF). #31 NARROWED, not retired: a real fail-closed inbound gate now exists and the binary concedes the finding's core claim in source, but the verified peer identity NEVER reaches the model and the in-process send path is never classified by the gate. A NEW CLASS OF LOCAL EXECUTION (#183): a remote session can drive shell commands on the user's local machine, bypassing the local permission system entirely and substituting the operating-system sandbox; inert on a stock install behind six independent gates. THREE COUNTING TRAPS fired, all one shape \u2014 a zero occurrence count read as novelty: names assembled at runtime never enter the string pool, one apparent new subsystem was a telemetry SPELLING change over code already present, and one of our own conclusions was a retroactive widening rather than a new trigger. Rule earned: a literal count of zero is evidence of absence from the string pool, never proof of absence from the code. PRIOR:   // session-81 (2026-07-22); binary moved v215 to v217; TWO new findings (#168 HIGH, #169 LOW), ZERO regressions, and the most remediation-dense window since v205. A PURE-JavaScript window: the compiled native sections are byte-identical across all three builds, the section count is unchanged, the embedded runtime build revision is flat, and there are zero embedded executable blobs. The binary grew +3.18 MiB but printable text grew only ~566 KB (17%); binary-to-strings growth ratios of 6.2x and 5.4x are consistent across both increments, the signature of embedded bytecode scaling with source rather than a hidden payload. Roughly eight modest subsystems, no new versioned API endpoints, no new egress hosts. All twelve standing finding anchors byte-stable; the memory secret-skip guard intact at a flat count; server-push config-cache keys 9 flat. Anthropic remediation this window included three sandbox escapes (symlink-followed workflow/scheduled-task writes, a symlinked working directory letting a background session escape its workspace, and worktree-isolated subagents redirecting git into the shared checkout via GIT_DIR/GIT_WORK_TREE), plus compound-command permission checking, a Windows network-path read prompt, invisible-Unicode permission validation, and a managed-endpoint telemetry override fix. One candidate was REFUTED rather than filed (a session-provenance environment string that skips an organization-policy entitlement layer, but no feature-flag read participates and local reachability is nil). PRIOR session-80 (2026-07-20); binary moved v212 to v215 across three releases; ZERO new findings, ZERO regressions, ZERO remediations; stable advanced v197 to v205. NOT a pure-JS window (the first since v198): all native sections moved and one runtime-internal section was dropped - fully ATTRIBUTED to a bundler/runtime BUILD-REVISION bump under an unchanged semantic version; the JavaScript share grew more than the binary did, because the native side shrank. DEFAULT-TRUE 43 FLAT, re-derived at MEMBER level (added and removed sets both empty). Server-pushed config-cache keys 9 flat. Two #110 raw-field-egress anchors dropped (10 to 6, 13 to 9) = a DRY refactor collapsing five inline plugin-command emits into one shared helper, all five still firing identical payloads - NOT a remediation; #110 stands. Two WATCH items, neither filed as a finding.
    latest_session: 80
  },
  ...
  version: {
    start: "v2.1.89",
    end: "v2.1.258",   // session-90: bumped together with range and current_binary, per the standing note. PRIOR session-87: v2.1.241.
    range: "v2.1.89 \u2192 v2.1.258",  // session-90: bumped together with end and current_binary -- the hero badge reads version.range, so all three move each release. PRIOR session-87 (2026-08-25): v221 to v241 DECODED in one pass \u2014 20 windows, 21 published versions. TEN issues filed (#176-#185); coverage moves to v2.1.241, the first move since v220. THE HEADLINE IS A METHOD GAP, NOT A FINDING (#185): every flag census this project had run read the DEFAULT out of the binary and assigned severity from it, which is the wrong question for a server-controlled configuration channel \u2014 the client's own local cache of what the server ACTUALLY SERVED had never been diffed against a census. 17 of the 55 newly-added default-off gates are switched ON for this account, four of them paths carrying externally-authored text into model context; the inverse check is clean (of 76 default-on gates only three are served off, none a permission decision). It reversed a severity call mid-session. SCOPING ERROR caught by the pass's own completeness critic: the planned sweep covered only the 35 new default-ON gates, but every wire-confirmed finding in the server-push injection lineage is an empty-or-false-default value the server FILLS IN, so the permissive state is reached by a push and not a withhold. The 55 default-OFF gates were then swept properly (54/54; adversarial verification returned 8 confirmed, 21 confirmed-but-overgraded, 2 refuted). TWO High on the permission/injection path: #176 (a runner applies server-supplied command-line arguments to its spawned child through a DENYLIST rather than an allowlist, leaving reachable one argument that appends server-controlled text to the child's system prompt and one that moves the child out of permission checking; a sibling bridge in the SAME binary uses a strict allowlist that predates it) and #182 (a server-controlled flag makes a new cross-session gate trust a field the SENDER supplies about itself, converting hold-for-review into automatic accept). #181 is a third (a default-off flag converting a mandatory human approval on destructive external-tool calls into a classifier decision \u2014 currently served OFF). #31 NARROWED, not retired: a real fail-closed inbound gate now exists and the binary concedes the finding's core claim in source, but the verified peer identity NEVER reaches the model and the in-process send path is never classified by the gate. A NEW CLASS OF LOCAL EXECUTION (#183): a remote session can drive shell commands on the user's local machine, bypassing the local permission system entirely and substituting the operating-system sandbox; inert on a stock install behind six independent gates. THREE COUNTING TRAPS fired, all one shape \u2014 a zero occurrence count read as novelty: names assembled at runtime never enter the string pool, one apparent new subsystem was a telemetry SPELLING change over code already present, and one of our own conclusions was a retroactive widening rather than a new trigger. Rule earned: a literal count of zero is evidence of absence from the string pool, never proof of absence from the code. PRIOR:  // unicode rightwards arrow. hero badge reads version.range; session-81 audited through v217 (npm latest=next), stable advanced v205 to v206
  ...
  cross_version_v161_v177: {
    ...
    watch_items: {
      ...
      read_gate_probe: "#152 live — flip the flag, verify Edit/Write proceeds on an un-read file, re-confirm no permission-gate interaction",
