...
window.VIZ_COUNTS = Object.freeze({
  ...
  security: {
    ...
    gh_label_counts: {
      critical: 14,
      high: 39,   // session-81: +1 = #168, a server-pushed configuration STRING with an empty default that is interpolated verbatim into the instruction text of the built-in multiple-choice question tool, reaching model context on the messages endpoint; validated only by a type check and a whitespace trim (no length cap, no schema), and read unconditionally while its sibling value is model-gated. Same primitive family as the earlier server-pushed-string findings. HIGH not CRITICAL under the promotion gate: call-site-derived, not yet wire-confirmed.
      ...
      labeled_total: 129,   // 14C + 39H + 61M + 15L
