...
window.VIZ_COUNTS = Object.freeze({
  ...
  security: {
    ...
    gh_label_counts: {
      ...
      labeled_total: 162,   // 15C + 51H + 77M + 19L (session-90, re-derived live 2026-09-02). PRIOR 142 = 15C + 41H + 68M + 18L (session-87)
      unlabeled_by_severity: 43,  // 205 - 162 (session-90). PRIOR 42 = 184 - 142 (session-87)
      repo_total: 205,  // sessions 88-90: +21. Severity tally re-derived DIRECTLY from issue labels, not incremented. PRIOR session-87: +15 across sessions 84-87. Severity tally re-derived DIRECTLY from issue labels, not incremented. PRIOR session-81: +3 total = #168 (HIGH) and #169 (LOW), BOTH genuine harness findings - the first non-zero-finding window since v208 - plus a tooling/process issue for the redaction gap found during the mirror sync. PRIOR session-80: +2 total = #166 (prompt-variant watch tracker) and #167 (a tooling/process issue recording a redaction-enforcer gap in this project OWN publish pipeline, found and fixed the same session). Harness findings this window: ZERO.
