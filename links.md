...
- **dev.to/picklepixel — "How I reverse engineered Claude Code's hidden pet system"**
  ...
  Confirms species name obfuscation and the Bun.hash vs FNV-1a branching.
...
- **mila-jovovich/mempalace** (GitHub)
  https://github.com/mila-jovovich/mempalace
  Local-first AI memory system using a palace metaphor (wings, rooms, drawers). 96.6% R@5 on LongMemEval, AAAK lossless 30x compression, 19 MCP tools, SQLite knowledge graph. Used in this project to persist Shingle's reactions across sessions.
  *Reliability: High — MIT-licensed, well-documented, runs entirely offline.*
...
### Hash & Identity
- Production uses Bun.hash (wyhash) on accountUuid + internal salt
- FNV-1a is the Node.js dev fallback only
- Both feed Mulberry32 PRNG for trait generation
- Species names obfuscated in binary
