...
import { resolveBones, readAuth, getUserAgent } from "../shared/config.mjs";
...
// BONES resolved from ~/.claude/shingle.json profile (native/mage/custom).
// Defaults to "native" (derived from userId hash). See tools/shared/bones.mjs.
...
async function callBuddyReact(transcript, reason, addressed) {
  ...
  const bones = resolveBones(config.accountUuid, config.userId);
