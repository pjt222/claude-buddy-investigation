...
**Date:** 2026-04-03 (last updated: 2026-06-30 — current binary v2.1.196, session 72)
...
**Update through v2.1.196 (sessions 61–72):** the rolling per-version audit carried the priority-finding set forward to v2.1.196 with **one upstream remediation** — the default-true sandbox-classifier fail-open inversion (#108) was **removed at v2.1.179** (0 occurrences v179 through v196; retained in the finding census as a remediated Critical). Two new findings were filed at v179/v187 (**#154** CRITICAL, **#155** HIGH — see PA17/PA18). The session-70 byte-test **demoted #127 Critical → High**. v2.1.193–195 (session-71) and v2.1.196 (session-72) added **zero new findings and zero regressions**; v196 carried a single hardening default-flip (an anti-MITM guard on artifact upload now defaults ON) plus a benign first-party plugin binary-asset provisioning path (a WATCH, not a finding — see the v196 note at the end of this section). The upstream source tag and bundled runtime have been **identical across v2.1.181 → v2.1.196** (every binary in that window is a rebuild of one source tag). Net direction across the full window: attack surface roughly flat, with one safety inversion removed.
...
**Persistence**: byte-stable v2.1.144 → v2.1.152 (the allowlist-flag literal count and the validator structure are byte-identical across 6 binary releases over 13 days; no remediation observed). The credential-return idiom remains present through v2.1.196; the v196 first-party plugin binary-asset path (see the v196 note below) is the **best-hardened** member of this plugin-credential family (#136/#140/#151) and does not relax #136.
