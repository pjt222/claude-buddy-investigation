...
**Live GH-label re-derivation (2026-07-22, post-v217, session-81)**: 14 critical / 39 high-priority / 60 medium-priority / 15 low-priority = **128 severity-labeled across 168 repo issues**. The v2.1.216–v2.1.217 window added **two new harness findings** — **#168** (HIGH, v2.1.216: a server-pushed configuration string reaches model context verbatim through the built-in multiple-choice question tool's instruction text) and **#169** (LOW, v2.1.217: the subagent recursion-depth ceiling became a server-pushed integer with no upper bound) — with **zero regressions**. #168 is now the most recent new HIGH, displacing #165 (server-push plugin-instruction override into model context, v207–v212).
...
### PA20 / #168 — Server-Pushed String → Multiple-Choice Question Tool Instructions (HIGH)
...
This is the same primitive family as the previously reported server-pushed-string findings **#106 / #154 / #165**. **Filed HIGH rather than CRITICAL** under the standing promotion gate: the reach is derived from the call site and is **not yet confirmed on the wire**.
...
>
> **Live tally: 14 critical / 39 high / 60 medium / 15 low across 168 issues** — the +1 high is **#168**, the +1 low is **#169**; re-derived directly from the issue tracker, not by arithmetic.
